...
export async function generateDivyaGuruResponse(userMessage: string): Promise<string> {
  ...
  try {
    ...
    const response = await fetch(`${POLLINATIONS_BASE}/${encoded}?model=openai&seed=42`, {
      signal: AbortSignal.timeout(15000),
    });
    if (response.ok) {
      const text = await response.text();
  ...
  try {
    const encoded = encodeURIComponent(prompt);
    const response = await fetch(`${POLLINATIONS_BASE}/${encoded}?model=mistral&seed=42`, {
      signal: AbortSignal.timeout(15000),
    });
    if (response.ok) {
      const text = await response.text();
