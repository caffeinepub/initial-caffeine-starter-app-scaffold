...
const SATTVIC_FOODS = {
  allowed: ['फल', 'दूध', 'दही', 'साबूदाना', 'मखाना', 'सिंघाड़े का आटा', 'आलू', 'शकरकंद', 'मेवे', 'नारियल'],
  restricted: ['अनाज', 'लहसुन', 'प्याज', 'मांस', 'मछली', 'अंडे', 'नमक (सेंधा नमक लें)', 'तामसिक भोजन'],
};
...
function CountdownTimer({ targetDate, name, emoji }: { targetDate: Date; name: string; emoji: string }) {
    ...
    <div className="bg-card border border-amber-500/30 rounded-2xl p-4 hover:scale-[1.02] transition-all duration-200">
      <div className="flex items-center gap-2 mb-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-xs">{targetDate.toLocaleDateString('hi-IN')}</p>
        </div>
      </div>
      {isPast ? (
        <p className="text-muted-foreground text-xs text-center">यह व्रत बीत चुका है</p>
      ) : (
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            <div key={t.label} className="bg-amber-900/30 rounded-xl p-2 text-center">
              <p className="text-amber-300 font-bold text-lg leading-none">{String(t.val).padStart(2, '0')}</p>
...
export default function VratDashboard() {
  ...
  const upcomingVrats = VRAT_DATES_2026.filter((v) => v.date.getTime() > new Date().getTime())
    ...
    <div className="animate-fade-in">
      ...
      <div className="px-4 space-y-4 pb-8">
        ...
        <div className="bg-gradient-to-r from-indigo-900/60 to-purple-900/60 border border-indigo-500/30 rounded-2xl p-4">
          ...
          <p className="text-indigo-300 text-xs mt-1">यह समय ध्यान, जाप और पूजा के लिए सर्वोत्तम है।</p>
        </div>

        {/* Next Vrat */}
        {nextVrat && (
          <div className="bg-gradient-to-r from-amber-900/60 to-orange-900/60 border border-amber-500/30 rounded-2xl p-4">
            <p className="text-amber-300 text-xs font-semibold mb-1 uppercase tracking-wider">अगला व्रत</p>
            ...
            <div className="mt-3">
              ...
              {nextVrat.mantras.map((m, i) => (
                <p key={i} className="text-white text-sm font-medium">{m}</p>
              ))}
            </div>
          </div>
        )}
        ...
        <div className="bg-card border border-border rounded-2xl p-4">
          <h2 className="text-foreground font-bold mb-3">🥗 सात्विक भोजन गाइड</h2>
            ...
            <div>
              ...
              <ul className="space-y-1">
                {SATTVIC_FOODS.allowed.map((food) => (
                  <li key={food} className="text-foreground text-xs flex items-center gap-1">
          ...
          <div className="space-y-2">
            {VRAT_CHECKLIST.map((item) => (
              ...
              >
                ...
                <span className={`text-sm flex-1 ${checkedTasks.includes(item.id) ? 'line-through text-muted-foreground' : 'text-foreground'}`}>
                  {item.task}
                </span>
                <span className={`text-lg ${checkedTasks.includes(item.id) ? 'text-green-400' : 'text-muted-foreground'}`}>
                  {checkedTasks.includes(item.id) ? '✅' : '⬜'}
                </span>
              </button>
            ))}
