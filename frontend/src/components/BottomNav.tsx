        ...
        <div className="flex items-center justify-around px-0.5 py-1">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={`text-base leading-none ${isActive ? 'animate-bounce-subtle' : ''}`}>
                  {item.emoji}
                </span>
                <span className={`text-[8px] font-medium leading-none truncate w-full text-center ${isActive ? 'text-gold-500' : ''}`}>
                  {item.label}
                </span>
                {isActive && (
