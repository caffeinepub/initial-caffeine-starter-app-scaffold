import { AlertCircle, Mic, Pause, Play, Square, Volume2 } from "lucide-react";
...
const ChatMessageComponent: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  const isThisPlaying =
    activeMessageId === message.id && narrationState === "playing";
  const isThisPaused =
    activeMessageId === message.id && narrationState === "paused";
  const isThisActive =
    activeMessageId === message.id &&
    narrationState !== "idle" &&
    narrationState !== "error";
  const isThisError =
    activeMessageId === message.id && narrationState === "error";
        ...
        {!isUser && isSupported && (
          <div className="flex items-center gap-1 mt-1.5 ml-1 flex-wrap">
            ...
            {isThisError && ttsError && (
              <span
                className="text-xs text-red-500 ml-1 max-w-[150px] truncate"
                title={ttsError}
              >
                {ttsError}
              </span>
