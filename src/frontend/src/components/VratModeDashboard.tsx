import { Calendar, CheckCircle, Circle, Moon, Sun } from "lucide-react";
...
function getNavratriProgress(): {
  ...
  if (today >= sharadStart && today <= sharadEnd) {
    const daysPassed =
      Math.round(
        (today.getTime() - sharadStart.getTime()) / (1000 * 60 * 60 * 24),
      ) + 1;
    return {
      current: daysPassed,
      total: 9,
...
export default function VratModeDashboard() {
  ...
  const [ekadashiCountdown, setEkadashiCountdown] =
    useState<CountdownInfo | null>(null);
  const [shivratriCountdown, setShivratriCountdown] =
    useState<CountdownInfo | null>(null);
    ...
    <div className="space-y-4 mt-4">
      ...
      <div className="bg-gradient-to-r from-indigo-950 to-purple-950 rounded-xl p-4 border border-indigo-800">
        ...
        <p className="text-indigo-300 text-xs mt-1">
          ध्यान और साधना का सर्वोत्तम समय
        </p>
      </div>
      ...
      <div className="bg-gradient-to-r from-amber-950 to-orange-950 rounded-xl p-4 border border-amber-800">
        ...
        {fastingChecked ? (
          <p className="text-green-400 text-xs mt-2 font-medium">
            ✓ आज का उपवास पूर्ण हुआ। जय माता दी! 🙏
          </p>
        ) : (
      ...
      <div className="bg-gradient-to-r from-rose-950 to-pink-950 rounded-xl p-4 border border-rose-800">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-rose-200 font-semibold text-sm">
            {navratri.name}
          </h3>
          {navratri.active && (
        ...
        {navratri.active ? (
          <>
            ...
            <div className="w-full bg-rose-900 rounded-full h-2">
              <div
                ...
                style={{
                  width: `${(navratri.current / navratri.total) * 100}%`,
                }}
              />
            </div>
          ...
          <p className="text-rose-300 text-sm">अगली नवरात्रि की प्रतीक्षा करें 🙏</p>
        )}
      </div>
        ...
        <div className="bg-gradient-to-b from-emerald-950 to-teal-950 rounded-xl p-3 border border-emerald-800">
          ...
          {ekadashiCountdown && (
            <>
              ...
              <p className="text-emerald-300 text-xs mt-1.5 leading-tight">
                {ekadashiCountdown.name}
              </p>
              <p className="text-emerald-600 text-xs">
                {formatDate(ekadashiCountdown.date)}
              </p>
            </>
          )}
        </div>
        ...
        <div className="bg-gradient-to-b from-slate-900 to-gray-950 rounded-xl p-3 border border-slate-700">
          ...
          {shivratriCountdown && (
            <>
              ...
              <p className="text-slate-300 text-xs mt-1.5 leading-tight">
                {shivratriCountdown.name}
              </p>
              <p className="text-slate-600 text-xs">
                {formatDate(shivratriCountdown.date)}
              </p>
            </>
          )}
        </div>
